<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="OverageReport.ascx.cs" Inherits="WaiverFile.WFSubscriptions.Controls.OverageReport" %>
...
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title">WaiverFile Overage Report
</h3>
    </div><!-- /.box-header -->
...
<rad:GridBoundColumn DataField="NumIncludedTotal" UniqueName="NumIncludedTotal" HeaderText="Included" FilterControlAltText="Filter NumIncludedTotal column" SortExpression="NumIncludedTotal" DataType="System.Int32"></rad:GridBoundColumn>
            ...
            <rad:GridBoundColumn DataField="numOver" FilterControlAltText="Filter numOver column" HeaderText="Over By" UniqueName="numOver" SortExpression="numOver" DataType="System.Int32" ReadOnly="True">
            </rad:GridBoundColumn>
...
<HeaderContextMenu EnableEmbeddedSkins="False"></HeaderContextMenu>
 ...
 <asp:SqlDataSource ID="sqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:WaiverFileDB %>"
        SelectCommand="select c.Customer_ID, s.url, s.name, c.firstName + ' ' + c.lastName as customername, c.email, per.NumIncludedTotal, per.NumUsed, per.NumUsed - per.NumIncludedTotal as numOver, per.PeriodBegins, per.PeriodEnds from tblWFSubscription sub 
...
inner join tblWFSubscriptionPeriod per on sub.id=per.WFSubscriptionID and per.id in (select top 1 AA.id from tblWFSubscriptionPeriod aa where aa.WFSubscriptionID=sub.ID order by aa.PeriodEnds desc)
     ...
     inner join tblWaiverSite s on s.WFSubscriptionID=sub.ID and s.ID in (select top 1 bb.ID from tblWaiverSite BB where bb.WFSubscriptionID=sub.ID order by url)
where NumUsed &gt; NumIncludedtotal and NumIncludedTotal&lt;&gt;-1 and ( NumIncludedTotal &lt;&gt; 0 or NumUsed &gt;= 30 )
