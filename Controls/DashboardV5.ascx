<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DashboardV5.ascx.cs" Inherits="WaiverFile.Controls.DashboardV5" %>
...
<script type="text/javascript">
    ...
    function playSoundNewWaiver() {
        playSound('<%=DASH_AUDIO_NEW_WAIVER%>');
    }
    function playSoundNewCheckIn() {
        playSound('<%=DASH_AUDIO_NEW_CHECKIN%>');
    }
    function playSoundNewFlag() {
        playSound('<%=DASH_AUDIO_NEW_FLAG%>');
    }
    ...
    function HandleDataResult(data, boxes, isFirstLoad) {
        ...
        for (id in data) {
            ...
            if (id.substring(0, 4) == 'EVT_') {
                BuildBoxDataEvent(data[id], boxes, $('.dasheventinner[data-eventid="' + id.substring(4, 40) + '"]').parents('.dashbox').data('dashboxid'), id.substring(4, 40), isFirstLoad);
            }
            else if (id.substring(0, 4) == 'meta') {
                ProcessMeta(data[id]);
            }
            else {
                BuildBoxData(data[id], boxes, id, isFirstLoad);
            }
        }
    ...
    function BuildBoxData(data, boxes, boxid, isFirstLoad) {
        var theBox = $('#dashmain').find(".dashbox[data-dashboxid='" + boxid + "']");
        var theBoxBody = theBox.find("tbody.dashboxbody");
        BuildBoxDataInner(data, boxes, boxid, theBox, theBoxBody, isFirstLoad);
        if (data.length == 0) {
    ...
    function BuildBoxDataEvent(data, boxes, boxid, eventid, isFirstLoad) {
    ...
    function BuildBoxDataInner(data, boxes, boxid, theBox, theBoxBody, isFirstLoad) {
        var newBlock = '';
        var hideExtra = false;
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (theBoxBody.find("tr[data-wfid='" + data[i]["waiverid"] + "']").length) {
                //ROW EXISTED, UPDATE THE CHECKIN STATUS IF IT CHANGED

                if (theBox.data('includecheckin') == 'True' || theBox.data('includecheckin') == 'true') {
                    var currPartRow = theBoxBody.find("tr[data-wfid='" + data[i]["waiverid"] + "'][data-wpid='" + data[i]["WaiverParticipantID"] + "']");
                    var isCurrentlyCheckedIn = $(currPartRow.find('input.checkin')[0]).is(':checked');
                    if (isCurrentlyCheckedIn != data[i]["IsCheckedIn"]) {
                        $(currPartRow.find('input.checkin')[0]).prop('checked', data[i]["IsCheckedIn"]); 
                        if (data[i]['IsCheckedIn']) {
                            playSoundNewCheckIn();
                        }
                    }
                }

                //UPDATE ANY FLAG STATUSES
                for (col in data[i]) {
                    if (col.indexOf('_WFFLAG_') != -1) {
                        ...
                        var currPartRow = theBoxBody.find("tr[data-wfid='" + data[i]["waiverid"] + "'][data-wpid='" + data[i]["WaiverParticipantID"] + "']");
                        var currFlagCheckbox = $(currPartRow).find('input[id="' + cbid + '"]');
                        if (currFlagCheckbox.length) {
                            if ($(currFlagCheckbox[0]).is(':checked') != data[i][col]) {
                                $(currFlagCheckbox[0]).prop('checked', data[i][col]);
                                if (data[i][col]) {
                                    playSoundNewFlag();
                                }
                            }
                ...
                for (col in data[i]) {
                    ...
                    if (col.indexOf('_WFFLAG_') != -1 || col.indexOf('__DT_FORMATTED') != -1 || col == 'RowNum' || col == "waiverid" || col == "WaiverParticipantID" || col == "IsCheckedIn" || col == 'WaiverParticipantEventID' || col == 'WPEDATESAVED'
                        || col == "IsSignee" || col== "IsCheckedIn") {
                        //DON'T DISPLAY THESE
                    ...
                    else if (col.substring(0, 13) == 'FormComplete_') {
                        newBlock += '<td>';

                        if (data[i][col] == true) {
                            newBlock += '<i class="fa fa-check text-success"></i>';
                        } else {
                            newBlock += '';
                        }
        ...
        if (reloadRunning == false) {
            ...
            if (pauseSync == 0) {
                ...
                $(boxSelector).each(function () {
                    ...
                    var grouped = $(this).data('grouped') == true;
                ...
                $.ajax({
                    type: "POST",
                    url: "/Controls/GetDashDataV5.ashx",
                    dataType: "json",
                    data: strRequest,
                    success: function (data) {
                        HandleDataResult(data, boxes, clearRowsFirst);
                    }
                });
