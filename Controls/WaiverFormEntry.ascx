...
<script type="text/javascript" src="/wp-content/Plugins/WaiverFile/Docs/jSignature/jSignature.js?v=4"></script>
        ...
        $('.btninitial').click(function () {
            ...
            if (sigAdoptDisabled || $('#' + $(this).data('hfid')).val() == '') {
                ...
                if ($('#<%=hfInitialData.ClientID%>').val().startsWith('__TYPED__')) {
                    $(this).replaceWith('<span class="initialtyped">' + $('#<%=hfInitialData.ClientID%>').val().substring(9) + '</span>');
                  } else {
...
<asp:Panel ID="pnlWaiver" runat="server" DefaultButton="">
        ...
        <div runat="server" id="div_address">
            ...
            <div class="form-group">
                <label  for="<%=txtZip.ClientID %>" class="control-label"><%=instance.GetLabel("LBL_ZIP", conn, site) %> <span class="text-danger" id="reqStart_addr4" runat="server">*</span>
                    ...
                    <asp:TextBox CssClass="form-control" ID="txtZip" runat="server" pattern="[0-9]*" MaxLength="5"></asp:TextBox>
            </div></div>
                </div>
    ...
    <div class="row">
       ...
       <a href="javascript:void(0);" class="btnsign btn btn-primary btn-lg btn-block" data-idx="-1" data-hfid="<%=hfSignatureData.ClientID %>" data-imgdiv="div_sig_main"><%=instance.GetLabel("BTN_SIGN_HERE_MAIN", Page, site) %></a>
           <%} %>
