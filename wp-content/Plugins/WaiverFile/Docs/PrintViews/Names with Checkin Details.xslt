...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:str="http://exslt.org/strings"  xmlns:set="http://exslt.org/sets" xmlns:dt="http://exslt.org/dates-and-times" exclude-result-prefixes="str dt set">
  ...
  <xsl:template mode="RenderMetaData" match="*">
    ...
    <Meta>
      <Name>Names with Checkin Details</Name>
      <Description>A view listing all participant names, signees, and checked-in status</Description>
    </Meta>
  ...
  <xsl:template mode="RenderView" match="*">
      ...
      <h3>
        <xsl:value-of select="//WaiverEvent[WaiverEventID=$WaiverEventID]/Name"/> - Checked In
      </h3>
      ...
      <table width="{$TableWidth}" style="background-color:#ffffff; " cellpadding="5">
        ...
        <xsl:for-each select="//Participant[IsCheckedIn='true' and ../WaiverEventID=$WaiverEventID]">
        ...
        <tr>
          <td colspan="2">
            <strong>
              ...
              <xsl:value-of select="count(//Participant[IsCheckedIn='true' and ../WaiverEventID=$WaiverEventID])"/>
            </strong>
          </td>
        </tr>
      </table>
      ...
      <h3>
        <xsl:value-of select="//WaiverEvent[WaiverEventID=$WaiverEventID]/Name"/> - Not Checked In
      </h3>
