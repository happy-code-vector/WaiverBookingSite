...
xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <xsl:template match="/">
    ...
    <xsl:param name="originalroot" select="/"/>
    ...
    <xsl:variable name="groupIDs">
      <data>
        <xsl:for-each select="set:distinct(/ArrayOfWFPlan/WFPlan/GroupID)">
          <id>
            <xsl:value-of select="."/>
          </id>
        </xsl:for-each>
    ...
    <xsl:for-each select="msxsl:node-set($groupIDs)/data/id">
      <xsl:variable name="groupid">
        <xsl:value-of select="."/>
      </xsl:variable>
      <xsl:variable name="groupname">
        <xsl:value-of select="$originalroot/ArrayOfWFPlan/WFPlan[GroupID=$groupid]/Name"/>
      </xsl:variable>

      
      <xsl:if test="count(set:distinct(/ArrayOfWFPlan/WFPlan/GroupID))&gt;1">
        <h3>
          <xsl:choose>
      ...
      <div class="row">
        ...
        <xsl:for-each select="$originalroot/ArrayOfWFPlan/WFPlan[ID!=$freeTrialWFPlanID and GroupID=$groupid]">
          ...
          <div class="col-xs-6 col-sm-3">
            <div class="panel panel-primary">
              ...
              <div class="panel-body text-center">
                ...
                <p>
                  <asp:Button runat="server" Text="Select" CssClass="btn btn-primary" data-mode="{Service}" CommandArgument="{ID}"></asp:Button>
                </p>
              </div>
            </div>
          </div>
