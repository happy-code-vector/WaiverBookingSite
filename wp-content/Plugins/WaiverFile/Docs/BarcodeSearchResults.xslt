...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:str="http://exslt.org/strings" xmlns:asp="remove" xmlns:diffgr="urn:schemas-microsoft-com:xml-diffgram-v1" xmlns:dt="http://exslt.org/dates-and-times" exclude-result-prefixes="str dt">
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <td>
          ...
          <!-- scanned? -->
          <xsl:choose>
            <xsl:when test="ScanDate!=''">
              <xsl:text>Scanned </xsl:text>
              <xsl:value-of select="dt:format-date(DateSigned,'MMM d, yyyy h:mm a')"/>
              <xsl:text> </xsl:text>
              <small>
              <asp:LinkButton runat="server" CommandName="UNDO_SCAN" CommandArgument="{WaiverID}" Text="Undo Scan" ></asp:LinkButton>
              </small>
            </xsl:when>
            <xsl:otherwise>
              <asp:LinkButton runat="server" CommandName="SCAN" CommandArgument="{WaiverID}" Text="Mark as Scanned" CssClass="btn btn-primary btn-sm"></asp:LinkButton>
            </xsl:otherwise>
          </xsl:choose>
